...
# import tf.compat.v1.layers.dense as tl
...
class ActorAgent(Agent):
    def __init__(self, sess, node_input_dim, job_input_dim, hid_dims, output_dim,
        ...
        logits = tf.compat.v1.log(self.node_act_probs)
        noise = tf.compat.v1.random_uniform(tf.shape(logits))
        self.node_acts = tf.argmax(logits - tf.compat.v1.log(-tf.compat.v1.log(noise)), 1)

        # job_acts [batch_size, num_jobs, 1]
        logits = tf.compat.v1.log(self.job_act_probs)
        noise = tf.compat.v1.random_uniform(tf.shape(logits))
        self.job_acts = tf.argmax(logits - tf.compat.v1.log(-tf.compat.v1.log(noise)), 2)
        ...
        self.selected_node_prob = tf.compat.v1.reduce_sum(tf.multiply(
            self.node_act_probs, self.node_act_vec),
            axis=1, keep_dims=True)

        # select job action probability
        self.selected_job_prob = tf.compat.v1.reduce_sum(tf.reduce_sum(tf.multiply(
            self.job_act_probs, self.job_act_vec),
            axis=2), axis=1, keep_dims=True)

        # actor loss due to advantge (negated)
        self.adv_loss = tf.reduce_sum(tf.multiply(
            tf.compat.v1.log(self.selected_node_prob * self.selected_job_prob + \
            self.eps), -self.adv))

        # node_entropy
        self.node_entropy = tf.reduce_sum(tf.multiply(
            self.node_act_probs, tf.compat.v1.log(self.node_act_probs + self.eps)))

        # prob on each job
        self.prob_each_job = tf.reshape(
            tf.compat.v1.sparse_tensor_dense_matmul(self.gsn.summ_mats[0],
        ...
        self.job_entropy = \
            ...
            tf.reduce_sum(tf.multiply(self.job_act_probs,
                tf.compat.v1.log(self.job_act_probs + self.eps)), axis=2)))
        ...
        # normalize entropy
        self.entropy_loss /= \
            (tf.compat.v1.log(tf.cast(tf.shape(self.node_act_probs)[1], tf.float32)) + \
            tf.compat.v1.log(float(len(self.executor_levels))))
    ...
    def actor_network(self, node_inputs, gcn_outputs, job_inputs,
        ...
        with tf.compat.v1.variable_scope(self.scope):
            ...
            node_hid_0 = tf.compat.v1.layers.dense(merge_node, 32, activation=act_fn)
            node_hid_1 =tf.compat.v1.layers.dense(node_hid_0, 16, activation=act_fn)
            node_hid_2 = tf.compat.v1.layers.dense(node_hid_1, 8, activation=act_fn)
            node_outputs = tf.compat.v1.layers.dense(node_hid_2, 1, activation=None)
            ...
            job_hid_0 =tf.compat.v1.layers.dense(expanded_state, 32, activation=act_fn)
            job_hid_1 = tf.compat.v1.layers.dense(job_hid_0, 16, activation=act_fn)
            job_hid_2 = tf.compat.v1.layers.dense(job_hid_1, 8, activation=act_fn)
            job_outputs = tf.compat.v1.layers.dense(job_hid_2, 1, activation=None)
